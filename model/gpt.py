...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        for i in range(num_blocks):
            self.transf_blocks.append(self.TransformerBlock(model_dim,   num_heads))
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        torch.manual_seed(0)
        embeddings = self.word_embedding(context) + self.pos_embedding(torch.arange(context.shape[1]))
        blocks = self.transf_blocks(embeddings)
        norm = self.final_norm(blocks)
        return torch.round(self.vocab_projection(norm), decimals=4)
